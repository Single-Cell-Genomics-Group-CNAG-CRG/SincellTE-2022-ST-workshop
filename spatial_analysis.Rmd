...
regions and thus overlapping spotscontain higher counts.
...
```{r}
table(colSums(as.matrix(sp_obj@assays$Spatial@counts)) == 0)
sp_obj <- subset(
  sp_obj,
...
sp_obj <- SCTransform(
  sp_obj,
  # assay to pull the count data from
  assay = "Spatial",
  # Number of subsampling Spots used to build NB regression, in this case use all
  ncells = ncol(sp_obj),
  # variable features to use after ranking by residual variance
...
```{r}
sp_obj <- RunPCA(sp_obj, npcs = 50)
ElbowPlot(sp_obj, ndims = 50)
```
...
ESR1 and ERBB2 are the most common mutations in breast cancer, so one way of 
annotating the tissue is by looking at ESR1 and ERBB2 positive/negative regions.

```{r fig.width=18, fig.height=6}
SpatialFeaturePlot(
  sp_obj, 
  features = c("ESR1", "ERBB2"), 
  alpha = c(0.1, 1)) + (SpatialDimPlot(
    sp_obj,
    label = TRUE) +
  NoLegend())
```
