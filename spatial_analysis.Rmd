...
general look we can load the raw HDF5 file instead.
```{r}
...
We will first go over some basic QC steps to remove poor quality spots and lowly
captured genes.
...
regions and thus contain spots with higher counts.
...
In the case of spatial data, high mitochondrial content is not an indicator of
bad quality spots. Therefore, on its own it is not a criterion to filter out spots.
...
normalization, identify variable features and scaling the counts.
...
```{r}
sp_obj <- RunPCA(sp_obj, npcs = 30)
ElbowPlot(sp_obj, ndims = 30)
```

We see an elbow at 10 PC, after that the standard deviations are pretty much
flat indicating that they aren't contributing much information. To reduce
computational resources and noise we proceed with the first 20 PCs.
...
sp_obj@meta.data <- sp_obj@meta.data %>%
  mutate(annotation = case_when(
    Spatial_snn_res.0.3 == 0 ~ "Fibrotic",
    Spatial_snn_res.0.3 == 1 ~ "Fibrotic",
    Spatial_snn_res.0.3 == 2 ~ "HER2-/ESR1+",
    Spatial_snn_res.0.3 == 3 ~ "HER2-/ESR1-",
    Spatial_snn_res.0.3 == 4 ~ "HER2+/ESR1-",
    Spatial_snn_res.0.3 == 5 ~ "HER2+/ESR1+",
    Spatial_snn_res.0.3 == 6 ~ "HER2-/ESR1-",
    Spatial_snn_res.0.3 == 7 ~ "HER2-/ESR1-",
    Spatial_snn_res.0.3 == 8 ~ "HER2-/ESR1-"
  ))
```

Look at the annotation
```{r fig.width=6, fig.height=6}
pal <- c("#ffb6db", "#004949", "#490092", "#920000", "#db6d00") # "#006ddb"
names(pal) <- c("Fibrotic", "HER2-/ESR1-", "HER2-/ESR1+", "HER2+/ESR1-", "HER2+/ESR1+")
...
Look at the spatial scatterpie by looking at cell types which are not present
